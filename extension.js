...
async function activate(context) {
	...
	(
		...
		('grails-commands-for-vscode.run-app', async function()
			{
...
function runApp(command){
	...
	let promise = new Promise(resolve => {
		let result = child.exec(command, {cwd: getWorkspaceDir(), name: 'Grails Run-App'});

		result.stdout.on("data", (data) => {
			linhaTeste = data;
			linhaLimpa = linhaTeste.match(/(http|https)?:\/\/.+/gi);
			if(linhaLimpa != null){
				console.error(linhaLimpa[0]);
				vscode.env.openExternal(vscode.Uri.parse(linhaLimpa[0]));
			}
			resolve(); 
		});
	...
	const terminal = vscode.window.createTerminal({cwd: getWorkspaceDir(), name: 'Grails Run-App'});
